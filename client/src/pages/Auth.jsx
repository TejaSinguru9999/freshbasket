...
function Auth() {
  ...
  return (
    <div className='h-[88vh] w-[100vw] flex justify-center items-center authContainer'>
      {isSignIn ? (
        <form className='authCard w-1/2 h-[75%] rounded-2xl p-12 py-8' onSubmit={handleSignIn}>
          ...
          <div className='flex flex-col mb-5'>
            ...
            <input
              className='px-5 py-3 rounded-xl outline-none border-gray-100 border-4'
              type='email'
              placeholder='Enter your registered GMail'
              required
              onChange={(e) => setSignIn((prev) => ({ ...prev, gmail: e.target.value }))}
            />
          </div>
          ...
          <div className='w-[100%]'>
            <button
              disabled={isAuth}
              className='w-15 bg-green-500 text-white px-5 py-3 rounded-lg mt-10 mx-auto block'
              type='submit'
            >
              {isAuth ? 'loading...' : 'Log In'}
            </button>
          </div>
          <p className='text-blue-200 cursor-pointer w-max ml-auto' onClick={() => setIsSignIn(false)}>
        ...
        <form className='authCard flex flex-col items-center gap-4 w-1/2 rounded-2xl py-8' onSubmit={handleRegister}>
          ...
          <div className='w-[100%]'>
            <button disabled={isAuth} className='w-15 bg-green-500 text-white px-5 py-3 rounded-lg mx-auto block' type='submit'>
              {isAuth ? 'loading...' : 'Create Account'}
            </button>
          </div>
          <p className='text-blue-200 cursor-pointer w-max ml-auto mr-6' onClick={() => setIsSignIn(true)}>
